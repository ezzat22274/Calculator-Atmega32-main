...
u8 LCD_SendString(u8* string, u8 Cursor)
{
	u8 ERROR = STD_TYPES_OK;
...
u8 LCD_SetPosition(u8 Line, u8 CharNum)
{
	u8  ERROR = STD_TYPES_OK;
	switch(Line)
	{
	case LCD_LINE_1 : LCD_SendCommand(0x80 + CharNum);break;
	case LCD_LINE_2 : LCD_SendCommand(0xC0 + CharNum);break;
	}
	return ERROR;
}
...
u8 LCD_SendFloatNumber(f32 num, u8 DecimalPlaces)
{
	if(num < 0)
	{
		LCD_SendChar('-');
